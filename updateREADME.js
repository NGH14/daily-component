...
function addProjectsToReadme() {
  ...
  const tableRegex = /\| Day \| Component\s*\| Main Concept\s*\|\s*\n\|[-\s|]+\|\s*\n((?:\|[^\n]*\|\n)*)/;
  ...
  const existingRows = tableMatch[1].split('\n').filter(row => row.trim() !== '' && row.includes('|'));
  ...
  const tableHeader = readmeContent.match(/\| Day \| Component\s*\| Main Concept\s*\|\s*\n\|[-\s|]+\|/)[0];
  ...
  projects.forEach(p => console.log(`- Day ${p.day}: ${p.projectName} (${p.fileExtensions.sort().join(', ')})`));
}
